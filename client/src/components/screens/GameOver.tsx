...
// ✅ Get the player's avatar or fallback to Dr. Dan
const defaultAvatar = localStorage.getItem('selectedAvatar') || '/assets/avatars/drdan2.png';

const GameOverPage = ({
  avatarUrl = defaultAvatar,
  codezillaUrl = '/assets/minions/codezilla2.png',
}) => {
  const navigate = useNavigate();

  useEffect(() => {
    // ✅ Add defeat background
  ...
  useEffect(() => {
    ...
    // ✅ Play roar with delay
    const roarDelay = setTimeout(() => {
      roar.play().catch((err) =>
        console.warn('🦖 Roar autoplay blocked:', err)
      );
    }, 2500);
  ...
  const handlePlayAgain = () => navigate('/map');
    ...
    <div className="game-over-page">
      <div className="game-over-container">
        ...
        <div className="game-over-actions">
          ...
          <button className="btn main-menu-btn" onClick={() => navigate('/leaderboard')}>
            Leaderboards
          </button>
        </div>
      </div>
